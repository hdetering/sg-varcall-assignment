...
The per-sequence GC content distribution shows two peaks, one at ~37% and another one at ~48%. The first peak is consistent with the GC content of the human nuclear genome (40.9%), whereas the second peak most likely corresponds to reads originating from the mitochondrial genome which has a higher GC content (44.4%).
...
Let's inspect the FN and FP calls more in detail.
...
#col_names = c("s","h_m_s","max_rss","max_vms","max_uss","max_pss","io_in","io_out","mean_load",NA)

df_perf <- tibble(filename = files) %>%
  mutate(sample = str_extract(filename, '((\\w+)\\.)?(\\w+)\\.perf', group = 2)) %>% # extract sample from filenames
  mutate(step = str_extract(filename, '((\\w+)\\.)?(\\w+)\\.perf', group = 3)) %>% # extract workflow step from filenames
